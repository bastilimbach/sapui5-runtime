...
if (process.env.HTTPS_PROXY) {
	let https_proxy_object = process.env.HTTPS_PROXY.split("://");
	proxyAuth = https_proxy_object[1].split("@")[0],
		hostname = https_proxy_object[1].split("@")[1].split(":")[0],
		port = https_proxy_object[1].split("@")[1].split(":")[1];
...
async function extractArchive(archive, targetPath) {
	...
	zip.on('ready', () => {
		...
		zip.extract(null, targetPath, (err, count) => {
